-- -------------------------------------------------------------
-- 
-- File Name: hdl_prj/hdlsrc/HDLTx/full_tx_ip_src_payload_full.vhd
...
-- -------------------------------------------------------------
-- 
-- Module: full_tx_ip_src_payload_full
...
ENTITY full_tx_ip_src_payload_full IS
...
END full_tx_ip_src_payload_full;


ARCHITECTURE rtl OF full_tx_ip_src_payload_full IS

  -- Component Declarations
  COMPONENT full_tx_ip_src_valid_to_ctrl_block1
    PORT( clk                             :   IN    std_logic;
          reset_x                         :   IN    std_logic;
          enb                             :   IN    std_logic;
  ...
  COMPONENT full_tx_ip_src_next_frame_to_ready
    PORT( clk                             :   IN    std_logic;
          reset_x                         :   IN    std_logic;
          enb_1_8_0                       :   IN    std_logic;
  ...
  COMPONENT full_tx_ip_src_p_scrambler
    PORT( clk                             :   IN    std_logic;
          reset_x                         :   IN    std_logic;
          enb                             :   IN    std_logic;
  ...
  COMPONENT full_tx_ip_src_reduced_ldpc
    PORT( clk                             :   IN    std_logic;
          reset_x                         :   IN    std_logic;
          enb                             :   IN    std_logic;
  ...
  COMPONENT full_tx_ip_src_puncturer
  ...
  COMPONENT full_tx_ip_src_payload_rep
    PORT( data_in                         :   IN    std_logic;
          ctrl_in_end                     :   IN    std_logic;
          ctrl_in_valid                   :   IN    std_logic;
          data_out                        :   OUT   std_logic;
  ...
  COMPONENT full_tx_ip_src_batid_to_bits_per_subcarrier
    PORT( bat_id                          :   IN    std_logic_vector(4 DOWNTO 0);  -- boolean [5]
          bits_per_subcarrier             :   OUT   std_logic_vector(3 DOWNTO 0)  -- ufix4
          );
  END COMPONENT;

  COMPONENT full_tx_ip_src_fec_rate_to_number
    PORT( Out1                            :   OUT   std_logic_vector(3 DOWNTO 0)  -- ufix4
          );
  END COMPONENT;

  COMPONENT full_tx_ip_src_psdu_size_to_payload_len
    PORT( clk                             :   IN    std_logic;
          reset_x                         :   IN    std_logic;
          enb_1_8_0                       :   IN    std_logic;
          enb                             :   IN    std_logic;
  ...
  COMPONENT full_tx_ip_src_tone_mapping
    PORT( clk                             :   IN    std_logic;
          reset_x                         :   IN    std_logic;
          enb                             :   IN    std_logic;
          data_in                         :   IN    std_logic;
  ...
  FOR ALL : full_tx_ip_src_valid_to_ctrl_block1
    USE ENTITY work.full_tx_ip_src_valid_to_ctrl_block1(rtl);

  FOR ALL : full_tx_ip_src_next_frame_to_ready
    USE ENTITY work.full_tx_ip_src_next_frame_to_ready(rtl);

  FOR ALL : full_tx_ip_src_p_scrambler
    USE ENTITY work.full_tx_ip_src_p_scrambler(rtl);

  FOR ALL : full_tx_ip_src_reduced_ldpc
    USE ENTITY work.full_tx_ip_src_reduced_ldpc(rtl);

  FOR ALL : full_tx_ip_src_puncturer
    USE ENTITY work.full_tx_ip_src_puncturer(rtl);

  FOR ALL : full_tx_ip_src_payload_rep
    USE ENTITY work.full_tx_ip_src_payload_rep(rtl);

  FOR ALL : full_tx_ip_src_batid_to_bits_per_subcarrier
    USE ENTITY work.full_tx_ip_src_batid_to_bits_per_subcarrier(rtl);

  FOR ALL : full_tx_ip_src_fec_rate_to_number
    USE ENTITY work.full_tx_ip_src_fec_rate_to_number(rtl);

  FOR ALL : full_tx_ip_src_psdu_size_to_payload_len
    USE ENTITY work.full_tx_ip_src_psdu_size_to_payload_len(rtl);

  FOR ALL : full_tx_ip_src_tone_mapping
    USE ENTITY work.full_tx_ip_src_tone_mapping(rtl);
  ...
  SIGNAL fec_rate_to_number_out1          : std_logic_vector(3 DOWNTO 0);  -- ufix4
  SIGNAL psdu_size_to_payload_len_out1    : std_logic_vector(28 DOWNTO 0);  -- ufix29
  SIGNAL len_in_ofdm_symbols_tmp          : unsigned(28 DOWNTO 0);  -- ufix29
  SIGNAL len_in_ofdm_symbols_tmp_1        : std_logic_vector(28 DOWNTO 0);  -- ufix29
...
BEGIN
  ...
  u_valid_to_ctrl : full_tx_ip_src_valid_to_ctrl_block1
    PORT MAP( clk => clk,
              reset_x => reset_x,
              enb => enb,
  ...
  u_next_frame_to_ready : full_tx_ip_src_next_frame_to_ready
    PORT MAP( clk => clk,
              reset_x => reset_x,
              enb_1_8_0 => enb_1_8_0,
  ...
  u_p_scrambler : full_tx_ip_src_p_scrambler
    PORT MAP( clk => clk,
              reset_x => reset_x,
              enb => enb,
  ...
  u_reduced_ldpc : full_tx_ip_src_reduced_ldpc
    PORT MAP( clk => clk,
              reset_x => reset_x,
              enb => enb,
  ...
  u_puncturer : full_tx_ip_src_puncturer
  ...
  u_payload_rep : full_tx_ip_src_payload_rep
  ...
  u_batid_to_bits_per_subcarrier : full_tx_ip_src_batid_to_bits_per_subcarrier
    PORT MAP( bat_id => bat_id,  -- boolean [5]
              bits_per_subcarrier => y_9  -- ufix4
              );

  u_fec_rate_to_number : full_tx_ip_src_fec_rate_to_number
    PORT MAP( Out1 => fec_rate_to_number_out1  -- ufix4
              );

  u_psdu_size_to_payload_len : full_tx_ip_src_psdu_size_to_payload_len
    PORT MAP( clk => clk,
              reset_x => reset_x,
              enb_1_8_0 => enb_1_8_0,
              enb => enb,
              ...
              fec_rate => fec_rate_to_number_out1,  -- ufix4
              payload_len_in_ofdm_symbols => psdu_size_to_payload_len_out1  -- ufix29
              );

  u_tone_mapping : full_tx_ip_src_tone_mapping
    PORT MAP( clk => clk,
              reset_x => reset_x,
              enb => enb,
              ...
              len_in_ofdm_symbols => len_in_ofdm_symbols_tmp_1,  -- ufix29
  ...
  len_in_ofdm_symbols_tmp <= unsigned(psdu_size_to_payload_len_out1);
