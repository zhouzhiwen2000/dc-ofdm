...
ARCHITECTURE rtl OF full_ofdm_modulator_ip_src_full_ofdm_modulator IS
  ...
  SIGNAL rd_8_reg_re                      : vector_of_signed12(0 TO 1);  -- sfix12_En10 [2]
  SIGNAL rd_8_reg_im                      : vector_of_signed12(0 TO 1);  -- sfix12_En10 [2]
...
BEGIN
  ...
  u_ofdm_modulator : full_ofdm_modulator_ip_src_ofdm_modulator_block
    PORT MAP( clk => clk,
              reset_x => reset_x,
              enb_1_2_0 => enb_1_2_0,
              data_in_re => out_new_re,  -- sfix12_En10
              data_in_im => out_new_im,  -- sfix12_En10
              valid_in => valid_new,
  ...
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        rd_8_reg_re <= (OTHERS => to_signed(16#000#, 12));
        rd_8_reg_im <= (OTHERS => to_signed(16#000#, 12));
      ELSIF enb_1_2_0 = '1' THEN
        rd_8_reg_im(0) <= ofdm_modulator_out1_im_signed;
        rd_8_reg_im(1) <= rd_8_reg_im(0);
        rd_8_reg_re(0) <= ofdm_modulator_out1_re_signed;
        rd_8_reg_re(1) <= rd_8_reg_re(0);
      END IF;
    END IF;
  END PROCESS rd_8_process;

  ofdm_modulator_out1_re_1 <= rd_8_reg_re(1);
  ofdm_modulator_out1_im_1 <= rd_8_reg_im(1);
