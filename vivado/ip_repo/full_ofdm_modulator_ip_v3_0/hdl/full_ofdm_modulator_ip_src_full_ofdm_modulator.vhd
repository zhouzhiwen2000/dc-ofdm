...
ARCHITECTURE rtl OF full_ofdm_modulator_ip_src_full_ofdm_modulator IS
  ...
  SIGNAL rd_3_reg_re                      : vector_of_signed12(0 TO 1);  -- sfix12_En10 [2]
  SIGNAL rd_3_reg_im                      : vector_of_signed12(0 TO 1);  -- sfix12_En10 [2]
...
BEGIN
  ...
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        rd_3_reg_re <= (OTHERS => to_signed(16#000#, 12));
        rd_3_reg_im <= (OTHERS => to_signed(16#000#, 12));
      ELSIF enb_1_2_0 = '1' THEN
        rd_3_reg_im(0) <= ofdm_modulator_out1_im_signed;
        rd_3_reg_im(1) <= rd_3_reg_im(0);
        rd_3_reg_re(0) <= ofdm_modulator_out1_re_signed;
        rd_3_reg_re(1) <= rd_3_reg_re(0);
      END IF;
    END IF;
  END PROCESS rd_3_process;

  ofdm_modulator_out1_re_1 <= rd_3_reg_re(1);
  ofdm_modulator_out1_im_1 <= rd_3_reg_im(1);
  ...
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        rd_4_reg <= (OTHERS => '0');
      ELSIF enb_1_2_0 = '1' THEN
        rd_4_reg(0) <= ofdm_modulator_out2;
        rd_4_reg(1) <= rd_4_reg(0);
      END IF;
    END IF;
  END PROCESS rd_4_process;

  ofdm_modulator_out2_1 <= rd_4_reg(1);
