-- -------------------------------------------------------------
-- 
-- File Name: hdl_prj/hdlsrc/HDLRxDemodulator/rx_demod_ip_src_find_peak.vhd
-- Created: 2024-09-22 18:53:32
-- 
-- Generated by MATLAB 24.1, HDL Coder 24.1, and Simulink 24.1
-- 
-- -------------------------------------------------------------


-- -------------------------------------------------------------
-- 
-- Module: rx_demod_ip_src_find_peak
-- Source Path: HDLRxDemodulator/rx_demodulator_full/ofdm_symbol_sync/find_peak
-- Hierarchy Level: 2
-- Model version: 1.58
-- 
-- -------------------------------------------------------------
LIBRARY IEEE;
USE IEEE.std_logic_1164.ALL;
USE IEEE.numeric_std.ALL;

ENTITY rx_demod_ip_src_find_peak IS
  PORT( clk                               :   IN    std_logic;
        reset_x                           :   IN    std_logic;
        enb_1_2_0                         :   IN    std_logic;
        enb                               :   IN    std_logic;
        enb_1_2_1                         :   IN    std_logic;
        data_in_re                        :   IN    std_logic_vector(15 DOWNTO 0);  -- sfix16_En14
        data_in_im                        :   IN    std_logic_vector(15 DOWNTO 0);  -- sfix16_En14
        M_in                              :   IN    std_logic_vector(15 DOWNTO 0);  -- sfix16_En14
        data_out_re                       :   OUT   std_logic_vector(15 DOWNTO 0);  -- sfix16_En14
        data_out_im                       :   OUT   std_logic_vector(15 DOWNTO 0);  -- sfix16_En14
        peak_sync                         :   OUT   std_logic
        );
END rx_demod_ip_src_find_peak;


ARCHITECTURE rtl OF rx_demod_ip_src_find_peak IS

  -- Component Declarations
  COMPONENT rx_demod_ip_src_SimpleDualPortRAM_generic_block1
    GENERIC( AddrWidth                    : integer;
             DataWidth                    : integer
             );
    PORT( clk                             :   IN    std_logic;
          enb_1_2_0                       :   IN    std_logic;
          wr_din                          :   IN    std_logic_vector(DataWidth - 1 DOWNTO 0);  -- generic width
          wr_addr                         :   IN    std_logic_vector(AddrWidth - 1 DOWNTO 0);  -- generic width
          wr_en                           :   IN    std_logic;  -- ufix1
          rd_addr                         :   IN    std_logic_vector(AddrWidth - 1 DOWNTO 0);  -- generic width
          dout                            :   OUT   std_logic_vector(DataWidth - 1 DOWNTO 0)  -- generic width
          );
  END COMPONENT;

  COMPONENT rx_demod_ip_src_Compare_To_Constant_block
    PORT( u                               :   IN    std_logic_vector(15 DOWNTO 0);  -- sfix16_En14
          y                               :   OUT   std_logic
          );
  END COMPONENT;

  -- Component Configuration Statements
  FOR ALL : rx_demod_ip_src_SimpleDualPortRAM_generic_block1
    USE ENTITY work.rx_demod_ip_src_SimpleDualPortRAM_generic_block1(rtl);

  FOR ALL : rx_demod_ip_src_Compare_To_Constant_block
    USE ENTITY work.rx_demod_ip_src_Compare_To_Constant_block(rtl);

  -- Signals
  SIGNAL data_in_re_1                     : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL data_in_im_1                     : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL data_in_re_2                     : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL data_in_im_2                     : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL Delay4_out1_re                   : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL Delay4_out1_im                   : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL Delay4_out1_re_1                 : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL Delay4_out1_im_1                 : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL Delay4_out1_re_2                 : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL Delay4_out1_im_2                 : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL mergedInput                      : unsigned(31 DOWNTO 0);  -- ufix32
  SIGNAL mergedDelay_regin                : unsigned(31 DOWNTO 0);  -- ufix32
  SIGNAL mergedDelay_waddr                : unsigned(6 DOWNTO 0);  -- ufix7
  SIGNAL mergedDelay_wrenb                : std_logic;  -- ufix1
  SIGNAL mergedDelay_raddr                : unsigned(6 DOWNTO 0);  -- ufix7
  SIGNAL mergedDelay_regout               : std_logic_vector(31 DOWNTO 0);  -- ufix32
  SIGNAL mergedDelay_regout_unsigned      : unsigned(31 DOWNTO 0);  -- ufix32
  SIGNAL mergedOutput                     : unsigned(31 DOWNTO 0);  -- ufix32
  SIGNAL slicedInput                      : unsigned(15 DOWNTO 0);  -- ufix16
  SIGNAL realOutput                       : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL slicedInput_1                    : unsigned(15 DOWNTO 0);  -- ufix16
  SIGNAL imagOutput                       : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL M_in_signed                      : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL data_in                          : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL data_in_1                        : std_logic_vector(15 DOWNTO 0);  -- ufix16
  SIGNAL Compare_To_Constant_out1         : std_logic;
  SIGNAL Compare_To_Constant_out1_1       : std_logic;
  SIGNAL Constant_out1                    : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL data_in_2                        : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL peak_i_1                         : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL peak_i_1_1                       : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL Delay2_bypass_reg                : signed(15 DOWNTO 0);  -- sfix16
  SIGNAL Delay2_out1                      : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL Delay2_out1_1                    : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL Delay_out1                       : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL Delay_out1_1                     : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL count_step                       : unsigned(6 DOWNTO 0);  -- ufix7
  SIGNAL count_from                       : unsigned(6 DOWNTO 0);  -- ufix7
  SIGNAL count_reset                      : unsigned(6 DOWNTO 0);  -- ufix7
  SIGNAL zero_1                           : std_logic;
  SIGNAL HDL_Counter_out1                 : unsigned(6 DOWNTO 0);  -- ufix7
  SIGNAL range_hit                        : std_logic;
  SIGNAL count                            : unsigned(6 DOWNTO 0);  -- ufix7
  SIGNAL need_to_wrap                     : std_logic;
  SIGNAL count_value                      : unsigned(6 DOWNTO 0);  -- ufix7
  SIGNAL count_value_1                    : unsigned(6 DOWNTO 0);  -- ufix7
  SIGNAL HDL_Counter_out1_1               : unsigned(6 DOWNTO 0);  -- ufix7
  SIGNAL en                               : std_logic;
  SIGNAL en_1                             : std_logic;
  SIGNAL Unit_Delay1_bypass_reg           : std_logic;  -- ufix1
  SIGNAL Unit_Delay1_out1                 : std_logic;
  SIGNAL Unit_Delay1_out1_1               : std_logic;
  SIGNAL peak_found                       : std_logic;
  SIGNAL peak_found_1                     : std_logic;
  SIGNAL Delay3_out1                      : std_logic;
  SIGNAL OR_out1                          : std_logic;
  SIGNAL en_2                             : std_logic;
  SIGNAL count_1                          : unsigned(6 DOWNTO 0);  -- ufix7
  SIGNAL new_peak                         : std_logic;
  SIGNAL new_peak_1                       : std_logic;
  SIGNAL count_2                          : unsigned(6 DOWNTO 0);  -- ufix7
  SIGNAL count_3                          : unsigned(6 DOWNTO 0);  -- ufix7
  SIGNAL HDL_Counter_bypass_reg           : unsigned(6 DOWNTO 0);  -- ufix7
  SIGNAL HDL_Counter_out1_2               : unsigned(6 DOWNTO 0);  -- ufix7
  SIGNAL count_hit                        : std_logic;
  SIGNAL peak_i                           : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL peak_i_2                         : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL Unit_Delay_bypass_reg            : signed(15 DOWNTO 0);  -- sfix16
  SIGNAL Unit_Delay_out1                  : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL Unit_Delay_out1_1                : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL HDL_Counter_out2                 : std_logic;
  SIGNAL HDL_Counter_out2_1               : std_logic;
  SIGNAL Delay1_bypass_reg                : std_logic;  -- ufix1
  SIGNAL peak_found_2                     : std_logic;
  SIGNAL peak_found_3                     : std_logic;
  SIGNAL peak_found_4                     : std_logic;
  SIGNAL new_peak_2                       : std_logic;
  SIGNAL Delay3_bypass_reg                : std_logic;  -- ufix1
  SIGNAL Delay3_out1_1                    : std_logic;
  SIGNAL OR1_out1                         : std_logic;

BEGIN
  u_ShiftRegisterRAM : rx_demod_ip_src_SimpleDualPortRAM_generic_block1
    GENERIC MAP( AddrWidth => 7,
                 DataWidth => 32
                 )
    PORT MAP( clk => clk,
              enb_1_2_0 => enb_1_2_0,
              wr_din => std_logic_vector(mergedDelay_regin),
              wr_addr => std_logic_vector(mergedDelay_waddr),
              wr_en => mergedDelay_wrenb,  -- ufix1
              rd_addr => std_logic_vector(mergedDelay_raddr),
              dout => mergedDelay_regout
              );

  u_Compare_To_Constant : rx_demod_ip_src_Compare_To_Constant_block
    PORT MAP( u => data_in_1,  -- sfix16_En14
              y => Compare_To_Constant_out1
              );

  data_in_re_1 <= signed(data_in_re);

  data_in_im_1 <= signed(data_in_im);

  Delay41_output_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        data_in_re_2 <= to_signed(16#0000#, 16);
        data_in_im_2 <= to_signed(16#0000#, 16);
      ELSIF enb_1_2_1 = '1' THEN
        data_in_re_2 <= data_in_re_1;
        data_in_im_2 <= data_in_im_1;
      END IF;
    END IF;
  END PROCESS Delay41_output_process;


  Delay4_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        Delay4_out1_re <= to_signed(16#0000#, 16);
        Delay4_out1_im <= to_signed(16#0000#, 16);
      ELSIF enb_1_2_0 = '1' THEN
        Delay4_out1_re <= data_in_re_2;
        Delay4_out1_im <= data_in_im_2;
      END IF;
    END IF;
  END PROCESS Delay4_process;


  Delay4_out1_re_1 <= Delay4_out1_re;

  Delay4_out1_im_1 <= Delay4_out1_im;

  Delay51_output_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        Delay4_out1_re_2 <= to_signed(16#0000#, 16);
        Delay4_out1_im_2 <= to_signed(16#0000#, 16);
      ELSIF enb_1_2_1 = '1' THEN
        Delay4_out1_re_2 <= Delay4_out1_re_1;
        Delay4_out1_im_2 <= Delay4_out1_im_1;
      END IF;
    END IF;
  END PROCESS Delay51_output_process;


  mergedInput <= unsigned(Delay4_out1_re_2) & unsigned(Delay4_out1_im_2);

  -- Input register for RAM-based shift register mergedDelay
  mergedDelay_reginc_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        mergedDelay_regin <= to_unsigned(0, 32);
      ELSIF enb_1_2_0 = '1' THEN
        mergedDelay_regin <= mergedInput;
      END IF;
    END IF;
  END PROCESS mergedDelay_reginc_process;


  -- Count limited, Unsigned Counter
  --  initial value   = 0
  --  step value      = 1
  --  count to value  = 124
  -- Write address counter for RAM-based shift register mergedDelay
  mergedDelay_wr_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        mergedDelay_waddr <= to_unsigned(16#00#, 7);
      ELSIF enb_1_2_0 = '1' THEN
        IF mergedDelay_waddr >= to_unsigned(16#7C#, 7) THEN 
          mergedDelay_waddr <= to_unsigned(16#00#, 7);
        ELSE 
          mergedDelay_waddr <= mergedDelay_waddr + to_unsigned(16#01#, 7);
        END IF;
      END IF;
    END IF;
  END PROCESS mergedDelay_wr_process;


  mergedDelay_wrenb <= '1';

  -- Count limited, Unsigned Counter
  --  initial value   = 1
  --  step value      = 1
  --  count to value  = 124
  -- Read address counter for RAM-based shift register mergedDelay
  mergedDelay_rd_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        mergedDelay_raddr <= to_unsigned(16#01#, 7);
      ELSIF enb_1_2_0 = '1' THEN
        IF mergedDelay_raddr >= to_unsigned(16#7C#, 7) THEN 
          mergedDelay_raddr <= to_unsigned(16#00#, 7);
        ELSE 
          mergedDelay_raddr <= mergedDelay_raddr + to_unsigned(16#01#, 7);
        END IF;
      END IF;
    END IF;
  END PROCESS mergedDelay_rd_process;


  mergedDelay_regout_unsigned <= unsigned(mergedDelay_regout);

  -- Output register for RAM-based shift register mergedDelay
  mergedDelay_regoutc_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        mergedOutput <= to_unsigned(0, 32);
      ELSIF enb_1_2_0 = '1' THEN
        mergedOutput <= mergedDelay_regout_unsigned;
      END IF;
    END IF;
  END PROCESS mergedDelay_regoutc_process;


  slicedInput <= mergedOutput(31 DOWNTO 16);

  realOutput <= signed(slicedInput);

  data_out_re <= std_logic_vector(realOutput);

  slicedInput_1 <= mergedOutput(15 DOWNTO 0);

  imagOutput <= signed(slicedInput_1);

  data_out_im <= std_logic_vector(imagOutput);

  M_in_signed <= signed(M_in);

  data_in <= M_in_signed;

  data_in_1 <= std_logic_vector(data_in);

  rd_2_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        Compare_To_Constant_out1_1 <= '0';
      ELSIF enb = '1' THEN
        Compare_To_Constant_out1_1 <= Compare_To_Constant_out1;
      END IF;
    END IF;
  END PROCESS rd_2_process;


  Constant_out1 <= to_signed(16#0000#, 16);

  rd_1_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        data_in_2 <= to_signed(16#0000#, 16);
      ELSIF enb = '1' THEN
        data_in_2 <= data_in;
      END IF;
    END IF;
  END PROCESS rd_1_process;


  
  peak_i_1 <= Constant_out1 WHEN Compare_To_Constant_out1_1 = '0' ELSE
      data_in_2;

  crp_out_delay_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        peak_i_1_1 <= to_signed(16#0000#, 16);
      ELSIF enb = '1' THEN
        peak_i_1_1 <= peak_i_1;
      END IF;
    END IF;
  END PROCESS crp_out_delay_process;


  Delay2_bypass_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        Delay2_bypass_reg <= to_signed(16#0000#, 16);
      ELSIF enb_1_2_1 = '1' THEN
        Delay2_bypass_reg <= peak_i_1_1;
      END IF;
    END IF;
  END PROCESS Delay2_bypass_process;

  
  Delay2_out1 <= peak_i_1_1 WHEN enb_1_2_1 = '1' ELSE
      Delay2_bypass_reg;

  Delay2_out1_1 <= Delay2_out1;

  Delay_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        Delay_out1 <= to_signed(16#0000#, 16);
      ELSIF enb_1_2_0 = '1' THEN
        Delay_out1 <= Delay2_out1;
      END IF;
    END IF;
  END PROCESS Delay_process;


  Delay_out1_1 <= Delay_out1;

  -- Count limited, Unsigned Counter
  --  initial value   = 0
  --  step value      = 1
  --  count to value  = 127
  count_step <= to_unsigned(16#01#, 7);

  count_from <= to_unsigned(16#00#, 7);

  count_reset <= to_unsigned(16#00#, 7);

  zero_1 <= '0';

  
  range_hit <= '1' WHEN HDL_Counter_out1 > to_unsigned(16#7E#, 7) ELSE
      '0';

  count <= HDL_Counter_out1 + count_step;

  
  count_value <= count WHEN need_to_wrap = '0' ELSE
      count_from;

  rd_7_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        count_value_1 <= to_unsigned(16#00#, 7);
      ELSIF enb = '1' THEN
        count_value_1 <= count_value;
      END IF;
    END IF;
  END PROCESS rd_7_process;


  rd_6_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        HDL_Counter_out1_1 <= to_unsigned(16#00#, 7);
      ELSIF enb = '1' THEN
        HDL_Counter_out1_1 <= HDL_Counter_out1;
      END IF;
    END IF;
  END PROCESS rd_6_process;


  crp_out_delay1_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        en_1 <= '0';
      ELSIF enb = '1' THEN
        en_1 <= en;
      END IF;
    END IF;
  END PROCESS crp_out_delay1_process;


  Unit_Delay1_bypass_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        Unit_Delay1_bypass_reg <= '0';
      ELSIF enb_1_2_1 = '1' THEN
        Unit_Delay1_bypass_reg <= en_1;
      END IF;
    END IF;
  END PROCESS Unit_Delay1_bypass_process;

  
  Unit_Delay1_out1 <= en_1 WHEN enb_1_2_1 = '1' ELSE
      Unit_Delay1_bypass_reg;

  Unit_Delay1_out1_1 <= Unit_Delay1_out1;

  peak_found_1 <= peak_found;

  OR_out1 <= peak_found_1 OR Delay3_out1;

  
  en <= Unit_Delay1_out1_1 WHEN OR_out1 = '0' ELSE
      Delay3_out1;

  rd_3_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        en_2 <= '0';
      ELSIF enb = '1' THEN
        en_2 <= en;
      END IF;
    END IF;
  END PROCESS rd_3_process;


  
  count_1 <= HDL_Counter_out1_1 WHEN en_2 = '0' ELSE
      count_value_1;

  rd_5_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        new_peak_1 <= '0';
      ELSIF enb = '1' THEN
        new_peak_1 <= new_peak;
      END IF;
    END IF;
  END PROCESS rd_5_process;


  
  count_2 <= count_1 WHEN new_peak_1 = '0' ELSE
      count_reset;

  crp_out_delay2_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        count_3 <= to_unsigned(16#00#, 7);
      ELSIF enb = '1' THEN
        count_3 <= count_2;
      END IF;
    END IF;
  END PROCESS crp_out_delay2_process;


  HDL_Counter_bypass_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        HDL_Counter_bypass_reg <= to_unsigned(16#00#, 7);
      ELSIF enb_1_2_1 = '1' THEN
        HDL_Counter_bypass_reg <= count_3;
      END IF;
    END IF;
  END PROCESS HDL_Counter_bypass_process;

  
  HDL_Counter_out1_2 <= count_3 WHEN enb_1_2_1 = '1' ELSE
      HDL_Counter_bypass_reg;

  HDL_Counter_out1 <= HDL_Counter_out1_2;

  
  need_to_wrap <= '1' WHEN HDL_Counter_out1 = to_unsigned(16#7F#, 7) ELSE
      '0';

  count_hit <= need_to_wrap OR range_hit;

  crp_out_delay5_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        peak_i_2 <= to_signed(16#0000#, 16);
      ELSIF enb = '1' THEN
        peak_i_2 <= peak_i;
      END IF;
    END IF;
  END PROCESS crp_out_delay5_process;


  Unit_Delay_bypass_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        Unit_Delay_bypass_reg <= to_signed(16#0000#, 16);
      ELSIF enb_1_2_1 = '1' THEN
        Unit_Delay_bypass_reg <= peak_i_2;
      END IF;
    END IF;
  END PROCESS Unit_Delay_bypass_process;

  
  Unit_Delay_out1 <= peak_i_2 WHEN enb_1_2_1 = '1' ELSE
      Unit_Delay_bypass_reg;

  Unit_Delay_out1_1 <= Unit_Delay_out1;

  crp_out_delay4_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        HDL_Counter_out2_1 <= '0';
      ELSIF enb = '1' THEN
        HDL_Counter_out2_1 <= HDL_Counter_out2;
      END IF;
    END IF;
  END PROCESS crp_out_delay4_process;


  Delay1_bypass_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        Delay1_bypass_reg <= '0';
      ELSIF enb_1_2_1 = '1' THEN
        Delay1_bypass_reg <= HDL_Counter_out2_1;
      END IF;
    END IF;
  END PROCESS Delay1_bypass_process;

  
  peak_found_2 <= HDL_Counter_out2_1 WHEN enb_1_2_1 = '1' ELSE
      Delay1_bypass_reg;

  peak_found_3 <= peak_found_2;

  peak_found <= peak_found_3;

  peak_found_4 <= peak_found;

  crp_out_delay3_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        new_peak_2 <= '0';
      ELSIF enb = '1' THEN
        new_peak_2 <= new_peak;
      END IF;
    END IF;
  END PROCESS crp_out_delay3_process;


  Delay3_bypass_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        Delay3_bypass_reg <= '0';
      ELSIF enb_1_2_1 = '1' THEN
        Delay3_bypass_reg <= new_peak_2;
      END IF;
    END IF;
  END PROCESS Delay3_bypass_process;

  
  Delay3_out1_1 <= new_peak_2 WHEN enb_1_2_1 = '1' ELSE
      Delay3_bypass_reg;

  Delay3_out1 <= Delay3_out1_1;

  OR1_out1 <= Delay3_out1 OR peak_found_4;

  
  peak_i <= Unit_Delay_out1_1 WHEN OR1_out1 = '0' ELSE
      Delay_out1_1;

  
  new_peak <= '1' WHEN Delay2_out1_1 > peak_i ELSE
      '0';

  
  HDL_Counter_out2 <= count_hit WHEN new_peak = '0' ELSE
      zero_1;

  peak_sync <= HDL_Counter_out2;

END rtl;

