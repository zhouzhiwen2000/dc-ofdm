-- -------------------------------------------------------------
-- 
-- File Name: hdl_prj/hdlsrc/HDLRxDemodulator/rx_demod_ip_src_rotate_90_block.vhd
-- Created: 2024-09-22 18:53:32
-- 
-- Generated by MATLAB 24.1, HDL Coder 24.1, and Simulink 24.1
-- 
-- -------------------------------------------------------------


-- -------------------------------------------------------------
-- 
-- Module: rx_demod_ip_src_rotate_90_block
-- Source Path: HDLRxDemodulator/rx_demodulator_full/constellation_descrambler/rotate_-90
-- Hierarchy Level: 2
-- Model version: 1.58
-- 
-- -------------------------------------------------------------
LIBRARY IEEE;
USE IEEE.std_logic_1164.ALL;
USE IEEE.numeric_std.ALL;
USE work.rx_demod_ip_src_rx_demodulator_full_pac.ALL;

ENTITY rx_demod_ip_src_rotate_90_block IS
  PORT( clk                               :   IN    std_logic;
        reset_x                           :   IN    std_logic;
        enb_1_2_0                         :   IN    std_logic;
        enb                               :   IN    std_logic;
        enb_1_2_1                         :   IN    std_logic;
        data_in_re                        :   IN    std_logic_vector(15 DOWNTO 0);  -- sfix16_En14
        data_in_im                        :   IN    std_logic_vector(15 DOWNTO 0);  -- sfix16_En14
        valid_in                          :   IN    std_logic;
        s1_in                             :   IN    std_logic;
        s2_in                             :   IN    std_logic;
        data_out_re                       :   OUT   std_logic_vector(15 DOWNTO 0);  -- sfix16_En14
        data_out_im                       :   OUT   std_logic_vector(15 DOWNTO 0);  -- sfix16_En14
        valid_out                         :   OUT   std_logic
        );
END rx_demod_ip_src_rotate_90_block;


ARCHITECTURE rtl OF rx_demod_ip_src_rotate_90_block IS

  -- Component Declarations
  COMPONENT rx_demod_ip_src_rotate_90
    PORT( clk                             :   IN    std_logic;
          reset_x                         :   IN    std_logic;
          enb                             :   IN    std_logic;
          s2                              :   IN    std_logic;
          s1                              :   IN    std_logic;
          data_in_re                      :   IN    std_logic_vector(15 DOWNTO 0);  -- sfix16_En14
          data_in_im                      :   IN    std_logic_vector(15 DOWNTO 0);  -- sfix16_En14
          data_out_re                     :   OUT   std_logic_vector(15 DOWNTO 0);  -- sfix16_En14
          data_out_im                     :   OUT   std_logic_vector(15 DOWNTO 0)  -- sfix16_En14
          );
  END COMPONENT;

  -- Component Configuration Statements
  FOR ALL : rx_demod_ip_src_rotate_90
    USE ENTITY work.rx_demod_ip_src_rotate_90(rtl);

  -- Signals
  SIGNAL en                               : std_logic;
  SIGNAL s1                               : std_logic;
  SIGNAL s2                               : std_logic;
  SIGNAL data_in_re_1                     : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL data_in_im_1                     : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL rd_0_reg_re                      : vector_of_signed16(0 TO 1);  -- sfix16_En14 [2]
  SIGNAL rd_0_reg_im                      : vector_of_signed16(0 TO 1);  -- sfix16_En14 [2]
  SIGNAL data_in_re_2                     : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL data_in_im_2                     : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL data_in_re_3                     : std_logic_vector(15 DOWNTO 0);  -- ufix16
  SIGNAL data_in_im_3                     : std_logic_vector(15 DOWNTO 0);  -- ufix16
  SIGNAL data_out_re_1                    : std_logic_vector(15 DOWNTO 0);  -- ufix16
  SIGNAL data_out_im_1                    : std_logic_vector(15 DOWNTO 0);  -- ufix16
  SIGNAL data_out_re_signed               : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL data_out_im_signed               : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL data_out_re_2                    : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL data_out_im_2                    : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL Delay2_bypass_reg_re             : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL Delay2_bypass_reg_im             : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL data_out_re_3                    : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL data_out_im_3                    : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL data_out_re_tmp                  : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL data_out_im_tmp                  : signed(15 DOWNTO 0);  -- sfix16_En14
  SIGNAL en_1                             : std_logic;
  SIGNAL Delay1_bypass_reg                : std_logic;  -- ufix1
  SIGNAL en_2                             : std_logic;

BEGIN
  u_rotate_90 : rx_demod_ip_src_rotate_90
    PORT MAP( clk => clk,
              reset_x => reset_x,
              enb => enb,
              s2 => s2,
              s1 => s1,
              data_in_re => data_in_re_3,  -- sfix16_En14
              data_in_im => data_in_im_3,  -- sfix16_En14
              data_out_re => data_out_re_1,  -- sfix16_En14
              data_out_im => data_out_im_1  -- sfix16_En14
              );

  en <= valid_in;

  s1 <= s1_in;

  s2 <= s2_in;

  data_in_re_1 <= signed(data_in_re);

  data_in_im_1 <= signed(data_in_im);

  rd_0_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        rd_0_reg_re <= (OTHERS => to_signed(16#0000#, 16));
        rd_0_reg_im <= (OTHERS => to_signed(16#0000#, 16));
      ELSIF enb_1_2_0 = '1' THEN
        rd_0_reg_im(0) <= data_in_im_1;
        rd_0_reg_im(1) <= rd_0_reg_im(0);
        rd_0_reg_re(0) <= data_in_re_1;
        rd_0_reg_re(1) <= rd_0_reg_re(0);
      END IF;
    END IF;
  END PROCESS rd_0_process;

  data_in_re_2 <= rd_0_reg_re(1);
  data_in_im_2 <= rd_0_reg_im(1);

  data_in_re_3 <= std_logic_vector(data_in_re_2);

  data_in_im_3 <= std_logic_vector(data_in_im_2);

  data_out_re_signed <= signed(data_out_re_1);

  data_out_im_signed <= signed(data_out_im_1);

  rd_1_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        data_out_re_2 <= to_signed(16#0000#, 16);
        data_out_im_2 <= to_signed(16#0000#, 16);
      ELSIF enb = '1' THEN
        data_out_re_2 <= data_out_re_signed;
        data_out_im_2 <= data_out_im_signed;
      END IF;
    END IF;
  END PROCESS rd_1_process;


  Delay2_bypass_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        Delay2_bypass_reg_re <= to_signed(16#0000#, 16);
        Delay2_bypass_reg_im <= to_signed(16#0000#, 16);
      ELSIF enb_1_2_1 = '1' THEN
        Delay2_bypass_reg_im <= data_out_im_2;
        Delay2_bypass_reg_re <= data_out_re_2;
      END IF;
    END IF;
  END PROCESS Delay2_bypass_process;

  
  data_out_re_3 <= data_out_re_2 WHEN enb_1_2_1 = '1' ELSE
      Delay2_bypass_reg_re;
  
  data_out_im_3 <= data_out_im_2 WHEN enb_1_2_1 = '1' ELSE
      Delay2_bypass_reg_im;

  data_out_re_tmp <= data_out_re_3;

  data_out_re <= std_logic_vector(data_out_re_tmp);

  data_out_im_tmp <= data_out_im_3;

  data_out_im <= std_logic_vector(data_out_im_tmp);

  en_1 <= en;

  Delay1_bypass_process : PROCESS (clk)
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        Delay1_bypass_reg <= '0';
      ELSIF enb_1_2_1 = '1' THEN
        Delay1_bypass_reg <= en_1;
      END IF;
    END IF;
  END PROCESS Delay1_bypass_process;

  
  en_2 <= en_1 WHEN enb_1_2_1 = '1' ELSE
      Delay1_bypass_reg;

  valid_out <= en_2;

END rtl;

