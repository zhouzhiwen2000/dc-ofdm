-- -------------------------------------------------------------
-- 
-- File Name: hdl_prj/hdlsrc/HDLRx/full_rx_ip_src_LDPC_Deco_Hc_12H.vhd
...
-- -------------------------------------------------------------
-- 
-- Module: full_rx_ip_src_LDPC_Deco_Hc_12H
-- Source Path: HDLRx/full_rx/h_rx_full/LDPC_Deco_Hc_12H
-- Hierarchy Level: 2
-- Model version: 1.101
-- 
-- LDPC Decoder
...
ENTITY full_rx_ip_src_LDPC_Deco_Hc_12H IS
...
END full_rx_ip_src_LDPC_Deco_Hc_12H;


ARCHITECTURE rtl OF full_rx_ip_src_LDPC_Deco_Hc_12H IS

  -- Component Declarations
  COMPONENT full_rx_ip_src_CodeParameters
    PORT( clk                             :   IN    std_logic;
          reset_x                         :   IN    std_logic;
          enb                             :   IN    std_logic;
          data                            :   IN    std_logic_vector(15 DOWNTO 0);  -- sfix16_En12
          start                           :   IN    std_logic;
          end_rsvd                        :   IN    std_logic;
          valid                           :   IN    std_logic;
          reset                           :   OUT   std_logic;  -- ufix1
          datacp                          :   OUT   std_logic_vector(15 DOWNTO 0);  -- sfix16_En12
  ...
  COMPONENT full_rx_ip_src_DecoderCore
    PORT( clk                             :   IN    std_logic;
          reset_x                         :   IN    std_logic;
          enb                             :   IN    std_logic;
  ...
  FOR ALL : full_rx_ip_src_CodeParameters
    USE ENTITY work.full_rx_ip_src_CodeParameters(rtl);

  FOR ALL : full_rx_ip_src_DecoderCore
    USE ENTITY work.full_rx_ip_src_DecoderCore(rtl);

  -- Signals
  SIGNAL sofVld                           : std_logic;
  SIGNAL in0_signed                       : signed(15 DOWNTO 0);  -- sfix16_En12
  SIGNAL dataIn                           : signed(15 DOWNTO 0);  -- sfix16_En12
  ...
  SIGNAL dataOut_signed                   : signed(15 DOWNTO 0);  -- sfix16_En12
  SIGNAL dataDTC                          : signed(17 DOWNTO 0);  -- sfix18_En12
  ...
  SIGNAL countVal                         : unsigned(8 DOWNTO 0);  -- ufix9
  SIGNAL maxCount                         : unsigned(8 DOWNTO 0);  -- ufix9
...
BEGIN
  u_Code_Parameters : full_rx_ip_src_CodeParameters
    PORT MAP( clk => clk,
              reset_x => reset_x,
              enb => enb,
              data => std_logic_vector(dataIn),  -- sfix16_En12
              start => startIn,
              end_rsvd => endIn,
              valid => validIn,
              reset => reset,  -- ufix1
              datacp => dataOut,  -- sfix16_En12
              validcp => valid,  -- ufix1
  ...
  u_Decoder_Core : full_rx_ip_src_DecoderCore
    PORT MAP( clk => clk,
              reset_x => reset_x,
              enb => enb,
              reset => reset,  -- ufix1
              data => std_logic_vector(dataDTC),  -- sfix18_En12
              valid => valid,  -- ufix1
              framevalid => frameValid,  -- ufix1
  ...
  dataDTC <= resize(dataOut_signed, 18);
  ...
  BEGIN
    IF clk'EVENT AND clk = '1' THEN
      IF reset_x = '1' THEN
        countVal <= to_unsigned(16#001#, 9);
      ELSIF enb = '1' THEN
        IF sofVld = '1' THEN 
          countVal <= to_unsigned(16#001#, 9);
        ELSIF vFrame = '1' THEN 
          IF countVal >= to_unsigned(16#150#, 9) THEN 
            countVal <= to_unsigned(16#001#, 9);
          ELSE 
            countVal <= countVal + to_unsigned(16#001#, 9);
          END IF;
        END IF;
      END IF;
    END IF;
